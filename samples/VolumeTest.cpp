...
int main()
{
    ...
    Turbo::Core::TImage *volume_image = nullptr;
    {
        Turbo::Core::TBuffer *volume_staging_buffer = new Turbo::Core::TBuffer(device, 0, Turbo::Core::TBufferUsageBits::BUFFER_TRANSFER_SRC, Turbo::Core::TMemoryFlagsBits::HOST_ACCESS_SEQUENTIAL_WRITE, sizeof(CData));
        ...
        Turbo::Core::TFence *volumn_fence = new Turbo::Core::TFence(device);

        queue->Submit(nullptr, nullptr, volumn_command_buffer, volumn_fence);

        volumn_fence->WaitUntil();

        delete volumn_fence;
        delete volume_staging_buffer;
        command_pool->Free(volumn_command_buffer);
    }
    Turbo::Core::TImageView *volume_image_view = new Turbo::Core::TImageView(volume_image, Turbo::Core::TImageViewType::IMAGE_VIEW_3D, volume_image->GetFormat(), Turbo::Core::TImageAspectBits::ASPECT_COLOR_BIT, 0, 1, 0, 1);
    ...
    std::vector<Turbo::Core::TImageView *> volumn_image_views;
    volumn_image_views.push_back(volume_image_view);
    graphics_pipeline_descriptor_set->BindData(0, 0, 0, volumn_image_views);

    Turbo::Core::TSampler *volume_sampler = new Turbo::Core::TSampler(device);
    std::vector<Turbo::Core::TSampler *> volume_samplers;
