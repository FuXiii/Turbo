...
const std::string MY_FRAG_SHADER_STR = ReadTextFile(asset_root + "/shaders/VolumnTest.frag");
    ...
    for (int k = 0; k < Dim[2]; k++)
    {
        for (int j = 0; j < Dim[1]; j++)
        {
            for (int i = 0; i < Dim[0]; i++)
            {
                ...
                {
                    // 白色
                    CData[i][j][k][0] = largeCubeC[0]; // 255.0 * largeCubeC[0];
                    CData[i][j][k][1] = largeCubeC[1]; // 255.0 * largeCubeC[1];
                    CData[i][j][k][2] = largeCubeC[2]; // 255.0 * largeCubeC[2];
                    CData[i][j][k][3] = largeCubeD;    // largeCubeD * 255.0;
                }
                else if (LinePS[0] <= 200)
                {
                    // 红色
                    CData[i][j][k][0] = middleSphereC[0]; // 255.0 * middleSphereC[0];
                    CData[i][j][k][1] = middleSphereC[1]; // 255.0 * middleSphereC[1];
                    CData[i][j][k][2] = middleSphereC[2]; // 255.0 * middleSphereC[2];
                    CData[i][j][k][3] = middleSphereD;    // middleSphereD * 255.0;
                }
                else
                {
                    // 黄色
                    CData[i][j][k][0] = smallCubeC[0]; // 255.0 * smallCubeC[0];
                    CData[i][j][k][1] = smallCubeC[1]; // 255.0 * smallCubeC[1];
                    CData[i][j][k][2] = smallCubeC[2]; // 255.0 * smallCubeC[2];
                    CData[i][j][k][3] = smallCubeD;    // smallCubeD * 255.0;
                }
                LinePS++;
            }
        }
...
int main()
{
    ...
    Turbo::Core::TRefPtr<Turbo::Core::TImage> volume_image = nullptr;
    {
        Turbo::Core::TRefPtr<Turbo::Core::TBuffer> volume_staging_buffer = new Turbo::Core::TBuffer(device, 0, Turbo::Core::TBufferUsageBits::BUFFER_TRANSFER_SRC, Turbo::Core::TMemoryFlagsBits::HOST_ACCESS_SEQUENTIAL_WRITE, sizeof(CData));
        void *volumn_ptr = volume_staging_buffer->Map();
        ...
        Turbo::Core::TRefPtr<Turbo::Core::TCommandBuffer> volumn_command_buffer = command_pool->Allocate();
        volumn_command_buffer->Begin();
        volumn_command_buffer->CmdTransformImageLayout(Turbo::Core::TPipelineStageBits::HOST_BIT, Turbo::Core::TPipelineStageBits::TRANSFER_BIT, Turbo::Core::TAccessBits::HOST_WRITE_BIT, Turbo::Core::TAccessBits::TRANSFER_WRITE_BIT, Turbo::Core::TImageLayout::UNDEFINED, Turbo::Core::TImageLayout::TRANSFER_DST_OPTIMAL, volume_image, Turbo::Core::TImageAspectBits::ASPECT_COLOR_BIT, 0, 1, 0, 1);
        {
        ...
        Turbo::Core::TRefPtr<Turbo::Core::TFence> volumn_fence = new Turbo::Core::TFence(device);

        queue->Submit(volumn_command_buffer, volumn_fence);

        volumn_fence->WaitUntil();

        command_pool->Free(volumn_command_buffer);
    }
    Turbo::Core::TRefPtr<Turbo::Core::TImageView> volume_image_view = new Turbo::Core::TImageView(volume_image, Turbo::Core::TImageViewType::IMAGE_VIEW_3D, volume_image->GetFormat(), Turbo::Core::TImageAspectBits::ASPECT_COLOR_BIT, 0, 1, 0, 1);
    ...
    layout << *my_vertex_shader << *my_fragment_shader;
    ...
    std::vector<Turbo::Core::TRefPtr<Turbo::Core::TImageView>> volumn_image_views;
    volumn_image_views.push_back(volume_image_view);
    graphics_pipeline_descriptor_set->BindData(0, 0, 0, volumn_image_views);

    Turbo::Core::TRefPtr<Turbo::Core::TSampler> volume_sampler = new Turbo::Core::TSampler(device);
    std::vector<Turbo::Core::TRefPtr<Turbo::Core::TSampler>> volume_samplers;
    volume_samplers.push_back(volume_sampler);
    graphics_pipeline_descriptor_set->BindData(0, 1, 0, volume_samplers);
