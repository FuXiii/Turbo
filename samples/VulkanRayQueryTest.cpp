...
const std::string VERT_SHADER_STR = ReadTextFile(asset_root + "/shaders/VulkanRayQueryTest.vert");
const std::string FRAG_SHADER_STR = ReadTextFile(asset_root + "/shaders/VulkanRayQueryTest.frag");
...
int main()
{
    ...
    Turbo::Core::TPhysicalDeviceFeatures physical_device_support_features = physical_device->GetDeviceFeatures();
    {
        ...
        if (physical_device_support_features.rayQuery)
        {
            std::cout << "Support ray query feature" << std::endl;
        }
        else
        {
            std::cout << "Not support ray query feature" << std::endl;
        }
    }

    if (!physical_device_support_features.accelerationStructure || !physical_device_support_features.rayQuery)
    {
        std::cout << "Please use a GPU which support acceleration structure and ray query" << std::endl;
        return 0;
    }
    ...
    physical_device_features.rayQuery = physical_device_support_features.rayQuery;
    ...
    for (Turbo::Core::TExtensionInfo &extension : physical_device_support_extensions)
    {
        ...
        else if (extension.GetExtensionType() == Turbo::Core::TExtensionType::VK_KHR_RAY_QUERY) // enable VK_KHR_ray_query extension
        {
            enable_device_extensions.push_back(extension);
        }
    }
