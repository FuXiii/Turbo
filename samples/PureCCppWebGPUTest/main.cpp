...
static void createPipelineAndBuffers()
{
    ...
    colorTarget.format = GetSwapChainFormat(device);
...
typedef bool (*pfn_redraw)();

EM_BOOL em_loop_callback(double /*time*/, void *userData)
{
    pfn_redraw redraw = (pfn_redraw)userData;
...
extern "C" int __main__(int /*argc*/, char * /*argv*/[])
{
    ...
    { // emscripten 的 html5_webgpu.h 中的函数

        queue = wgpuDeviceGetQueue(device); // WebGPU 标准函数
        ...
        emscripten_request_animation_frame_loop(em_loop_callback, (void *)redraw);
    }
    return 0;
}
