...
BOTTOM_LEVEL_ACCELERATION_STRUCTURE CreateBottomLevelAccelerationStructure(Turbo::Core::TRefPtr<Turbo::Core::TDevice> device, Turbo::Core::TRefPtr<Turbo::Core::TDeviceQueue> queue, MESH &mesh)
{
    ...
    assert(mesh.POSITION_data.size() == mesh.NORMAL_data.size() && mesh.NORMAL_data.size() == mesh.TEXCOORD_data.size());

    std::vector<VERTEX> vertexs;
    for (size_t vertex_index = 0; vertex_index < mesh.POSITION_data.size(); vertex_index++)
    {
        VERTEX temp_vertex = {};
        temp_vertex.position = mesh.POSITION_data[vertex_index];
        temp_vertex.normal = mesh.NORMAL_data[vertex_index];
        temp_vertex.texcoord = mesh.TEXCOORD_data[vertex_index];
...

    {
        ...
        {
            max_primitive_counts[index] = mesh.INDICES_data.size() / 3;
        }
        ...
        vk_acceleration_structure_build_range_info_khr.primitiveCount = mesh.INDICES_data.size() / 3;
...
TOP_LEVEL_ACCELERATION_STRUCTUR CreateTopLevelAccelerationStructure(Turbo::Core::TRefPtr<Turbo::Core::TDevice> device, Turbo::Core::TRefPtr<Turbo::Core::TDeviceQueue> queue, std::vector<NODE> &nodes, std::vector<BOTTOM_LEVEL_ACCELERATION_STRUCTURE> bottomLevelAccelerationStructures)
{
    ...
    {
        glm::mat4 acceleration_structure_model = glm::transpose(nodes[instance_index].matrix);
        ...
        bottom_level_acceleration_structure_device_address_info_khr.accelerationStructure = bottomLevelAccelerationStructures[nodes[instance_index].meshIndex].bottomLevelAccelerationStructure;

        VkDeviceAddress bottom_level_acceleration_structure_device_address = device->GetDeviceDriver()->vkGetAccelerationStructureDeviceAddressKHR(device->GetVkDevice(), &bottom_level_acceleration_structure_device_address_info_khr);
        ...
        vk_acceleration_structure_instance_khr.instanceCustomIndex = instance_index;
    ...
    VkResult result = device->GetDeviceDriver()->vkCreateAccelerationStructureKHR(device->GetVkDevice(), &top_level_acceleration_structure_create_info_khr, vk_allocation_callbacks, &top_level_acceleration_structure_khr);
    if (result != VkResult::VK_SUCCESS)
    {
    ...
    // Top Level Acceleration Structure Compact
    {
        ...
        result = device->GetDeviceDriver()->vkCreateQueryPool(device->GetVkDevice(), &vk_query_pool_create_info, vk_allocation_callbacks, &query_pool);
        if (result != VK_SUCCESS)
        {
            std::cout << "Create VkQueryPool Failed" << std::endl;
        }
        ...
        result = device->GetDeviceDriver()->vkCreateAccelerationStructureKHR(device->GetVkDevice(), &compact_vk_acceleration_structure_create_info_khr, vk_allocation_callbacks, &compact_acceleration_structure_khr);
        if (result != VkResult::VK_SUCCESS)
        {
            std::cout << "vkCreateAccelerationStructureKHR create compact VkAccelerationStructureKHR Failed" << std::endl;
        }
    ...
    top_level_acceleration_structure.topLevelAccelerationStructure = top_level_acceleration_structure_khr;
    top_level_acceleration_structure.instancBuffer = instance_buffer; // 实体缓存
...
int main()
{
    ...
    std::vector<NODE> nodes;
    {
        ...
        for (auto node_index : default_scene.nodes)
        {
            ...
            glm::mat4::col_type v0(node_matrix[0], node_matrix[1], node_matrix[2], node_matrix[3]);
            glm::mat4::col_type v1(node_matrix[4], node_matrix[5], node_matrix[6], node_matrix[7]);
        ...
        for (auto &mesh : model.meshes)
        {
            ...
            auto &bace_color_factor = material.pbrMetallicRoughness.baseColorFactor;
            auto &emissive_factor = material.emissiveFactor;

            MATERIAL temp_material = {};
            temp_material.pbrBaseColorFactor.r = bace_color_factor[0];
            temp_material.pbrBaseColorFactor.g = bace_color_factor[1];
            temp_material.pbrBaseColorFactor.b = bace_color_factor[2];
            temp_material.emissiveFactor.r = emissive_factor[0];
            temp_material.emissiveFactor.g = emissive_factor[1];
            temp_material.emissiveFactor.b = emissive_factor[2];
            ...
            temp_mesh.material = std::move(temp_material);
    ...
    for (MESH &mesh_item : meshes)
    {
        ...
        bottom_level_acceleration_structures.push_back(bottom_level_acceleration_structure);
    }
    ...
    {
        auto bottom_level_acceleration_structure_item = bottom_level_acceleration_structures[node_item.meshIndex];
        ...
        device_local_vertex_buffer_device_address_info.sType = VkStructureType::VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
        device_local_vertex_buffer_device_address_info.pNext = nullptr;
        device_local_vertex_buffer_device_address_info.buffer = bottom_level_acceleration_structure_item.vertexBuffer->GetVkBuffer();
        ...
        device_local_index_buffer_device_address_info.sType = VkStructureType::VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
        device_local_index_buffer_device_address_info.pNext = nullptr;
        device_local_index_buffer_device_address_info.buffer = bottom_level_acceleration_structure_item.indexBuffer->GetVkBuffer();
    ...
    Turbo::Core::TRefPtr<Turbo::Core::TBuffer> sbt_buffer = nullptr;
    {
        ...
        vk_write_descriptor_set_acceleration_structure_khr.pAccelerationStructures = &top_level_acceleration_structure.topLevelAccelerationStructure;
        ...
        case Turbo::Core::TResult::SUCCESS: {
            ...
            if (is_ray_tracing)
            {
                // ray tracing commands
                {
                    ...
                    VkBool32 vk_is_show_barycentrics = VK_FALSE;
                    if (is_show_barycentrics)
                    {
            ...
            {
                command_buffer->CmdTransformImageLayout(Turbo::Core::TPipelineStageBits::TOP_OF_PIPE_BIT, Turbo::Core::TPipelineStageBits::TRANSFER_BIT, Turbo::Core::TAccessBits::ACCESS_NONE, Turbo::Core::TAccessBits::TRANSFER_WRITE_BIT, Turbo::Core::TImageLayout::UNDEFINED, Turbo::Core::TImageLayout::TRANSFER_DST_OPTIMAL, swpachain_framebuffers[current_image_index]->GetAttachments()[0]);
                command_buffer->CmdClearColorImage(swpachain_framebuffers[current_image_index]->GetAttachments()[0]->GetImage(), Turbo::Core::TImageLayout::TRANSFER_DST_OPTIMAL, 0, 0, 0, 0);
                command_buffer->CmdTransformImageLayout(Turbo::Core::TPipelineStageBits::TRANSFER_BIT, Turbo::Core::TPipelineStageBits::COLOR_ATTACHMENT_OUTPUT_BIT, Turbo::Core::TAccessBits::ACCESS_NONE, Turbo::Core::TAccessBits::COLOR_ATTACHMENT_WRITE_BIT, Turbo::Core::TImageLayout::TRANSFER_DST_OPTIMAL, Turbo::Core::TImageLayout::COLOR_ATTACHMENT_OPTIMAL, swpachain_framebuffers[current_image_index]->GetAttachments()[0]);
            }
