...
void ply_get_property(PlyFile *plyfile, const char *elem_name, PlyProperty *prop)
{
    ...
    if (elem)
    {
        ...
        for (std::vector<std::string>::iterator it_property = tokens.begin(); !prop_ptr && it_property != tokens.end(); ++it_property)
        {
...
PlyOtherElems *ply_get_other_element(PlyFile *plyfile, char *elem_name, int elem_count)
{
    ...
    {
        other_elems = plyfile->other_elems;
        other_elems->other_list = (OtherElem *)realloc(other_elems->other_list, sizeof(OtherElem) * other_elems->num_elems + 1);
...
void get_binary_item(PlyFile *plyfile, int type, int *int_val, unsigned int *uint_val, double *double_val)
{
    ...
    case PLY_CHAR:
        ...
        if (result < 1)
        {
            throw std::runtime_error("Error in reading PLY file."
                                     "fread not succeeded.");
        }
        *int_val = *((char *)ptr);
        *uint_val = *int_val;
