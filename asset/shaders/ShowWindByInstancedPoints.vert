...
layout(set = 0, binding = 2, rgba32f) uniform image2D myParticalPosTex; //(x, y, z, lifetime)
...
void main()
{
    ...
    outSamplePos = imageLoad(myParticalPosTex, tex_coord);
    if (outSamplePos.w < time)
    {
        float random_u = hash(dot(outSamplePos.xy + vec2(901.2634280527, 458.97914471036), vec2(135.6379606213, 383.5357022848)));
        float random_v = hash(dot(outSamplePos.yx + vec2(640.0754402848, 813.1273531719), vec2(375.4331612936, 198.0555356584)));

        vec2 start_partical_pos = (uv + vec2(random_u, random_v)) * 0.5;
        outSamplePos = vec4(start_partical_pos, 0, outSamplePos.w);
        outSamplePos.w = time + (random_u + random_v) * 2;
    }
    ...
    vec4 next_sample_pos = vec4(outSamplePos.xy + current_flow_dir * 0.00003 * my_push_constants.speed, 0, outSamplePos.w);

    imageStore(myParticalPosTex, tex_coord, next_sample_pos);
