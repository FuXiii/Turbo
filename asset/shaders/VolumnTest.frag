...
layout(set = 0, binding = 0) uniform texture3D volumeNoise;
...
vec4 RayMarchingBoundingBox(vec3 origin, vec3 dir, BoundingBox boundingBox, float coverage, float time)
{
    ...
    if (is_intersect)
    {
        ...
        for (int i = 0; i < max_step; ++i)
        {
            ...
            vec4 samplec = texture(sampler3D(volumeNoise, mySampler), sample_point, 0);
...
void main()
{
    ...
    vec4 color = RayMarchingBoundingBox(cameraPos, rayDir, bounding_box, coverage, iTime);
