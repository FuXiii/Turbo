...
//
        ...
        auto vk_physical_device = physicalDevice->GetVkPhysicalDevice();

        struct TFormatRange
        {
            uint32_t first_format; // VkFormat
            uint32_t last_format;  // VkFormat
        };

        auto format_ranges = std::array{
            TFormatRange{1, 184},
            TFormatRange{1000156000, 1000156033},
            TFormatRange{1000330000, 1000330003},
            TFormatRange{1000340000, 1000340001},
            TFormatRange{1000066000, 1000066013},
            TFormatRange{1000470000, 1000470001},
            TFormatRange{1000054000, 1000054007},
            TFormatRange{1000464000, 1000464000},
        };
        ...
        for (auto &item : format_ranges)
        {
            for (uint32_t format = item.first_format; format <= item.last_format; format++)
            {
                ...
                pfn_vk_get_physical_device_format_properties(vk_physical_device, static_cast<VkFormat>(format), &vk_format_properties);

                if (vk_format_properties.bufferFeatures || vk_format_properties.linearTilingFeatures || vk_format_properties.optimalTilingFeatures)
                {
                    TFormatInfo format_info(static_cast<TFormatType>(format), vk_format_properties);
                    formats.push_back(format_info);
                }
            }
        }
    }
