...
size_t Turbo::Core::TExtensionInfo::GetPhysicalDeviceExtensionCount(TPhysicalDevice *physicalDevice, TLayerInfo *layer)
{
    ...
    if (physicalDevice != nullptr && physicalDevice->GetVkPhysicalDevice() != VK_NULL_HANDLE)
    {
        ...
        do
        {
            if (layer == nullptr || (layer != nullptr && layer->GetName().empty()))
            {
                result = pfn_vk_enumerate_device_extension_properties(physicalDevice->GetVkPhysicalDevice(), nullptr, &physical_device_extension_count, nullptr);
            }
            else
            {
                result = pfn_vk_enumerate_device_extension_properties(physicalDevice->GetVkPhysicalDevice(), layer->GetName().c_str(), &physical_device_extension_count, nullptr);
            }
...
std::vector<Turbo::Core::TExtensionInfo> Turbo::Core::TExtensionInfo::GetPhysicalDeviceExtensions(TPhysicalDevice *physicalDevice, TLayerInfo *layer)
{
    ...
    do
    {
        ...
        if (physical_device_extension_count > 0)
        {
            ...
            if (layer == nullptr || (layer != nullptr && layer->GetName().empty()))
            {
                result = pfn_vk_enumerate_device_extension_properties(physicalDevice->GetVkPhysicalDevice(), nullptr, &physical_device_extension_count, extension_propertiess.data());
            }
            else
            {
                result = pfn_vk_enumerate_device_extension_properties(physicalDevice->GetVkPhysicalDevice(), layer->GetName().c_str(), &physical_device_extension_count, extension_propertiess.data());
            }
