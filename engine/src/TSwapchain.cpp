...
{
    if (device != nullptr && surface != nullptr && device->GetPhysicalDevice() != nullptr &&
        surface->GetPhysicalDevice() != nullptr &&
        device->GetPhysicalDevice()->GetVkPhysicalDevice() != VK_NULL_HANDLE &&
        surface->GetPhysicalDevice()->GetVkPhysicalDevice() != VK_NULL_HANDLE &&
        surface->GetVkSurfaceKHR() != VK_NULL_HANDLE)
    {
        if (device->GetPhysicalDevice()->GetVkPhysicalDevice() == surface->GetPhysicalDevice()->GetVkPhysicalDevice() &&
            device->IsEnabledExtension(TExtensionType::VK_KHR_SWAPCHAIN))
        {
            ...
            for (size_t surface_format_index = 0; surface_format_index < surface_format_count; surface_format_index++)
            {
                ...
                if (format == VkFormat::VK_FORMAT_R8G8B8A8_SRGB)
                {
                    ...
                    target_surface_color_space =
                        surface_formats[surface_format_index].GetColorSpace().GetVkColorSpaceKHR();
                    break;
                }
            ...
            if (target_surface_format != VkFormat::VK_FORMAT_UNDEFINED &&
                target_surface_color_space == VkColorSpaceKHR::VK_COLORSPACE_SRGB_NONLINEAR_KHR)
            {
            ...
            VkResult result =
                vkCreateSwapchainKHR(device->GetVkDevice(), &swapchain_create_info, nullptr, &this->vkSwapchainKHR);
